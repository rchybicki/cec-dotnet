    ...
    partial class CECTray : AsyncForm
    {
        ...
        protected override void WndProc(ref Message msg)
        {
            ...
            if (msg.Msg == WM_POWERBROADCAST)
            {
                switch (msg.WParam.ToInt32())
                {
                    ...
                    {
                        POWERBROADCAST_SETTING pwr =
                            (POWERBROADCAST_SETTING) Marshal.PtrToStructure(msg.LParam, typeof(POWERBROADCAST_SETTING));
        ...
        private void BSaveLogClick(object sender, EventArgs e)
        {
            ...
            if (dialog.ShowDialog() == DialogResult.OK)
            {
                ...
                if (!fs.CanWrite)
                {
                    MessageBox.Show(string.Format(Resources.cannot_open_for_writing, dialog.FileName),
                        Resources.app_name, MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
                else
                {
                    ...
                    fs.Close();
                    fs.Dispose();
                    MessageBox.Show(string.Format(Resources.log_stored_as, dialog.FileName), Resources.app_name,
